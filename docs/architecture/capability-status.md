...
| Aether Calls (LiveKit) | yes | yes | no | no | Since `b292883`..: primary backend — LiveKit SDK 2.18.2, dev call service (`call-service/`, Node, room-join JWTs, per-invite DEV e2ee key), backend chooser, animated active-call return icon, no floating call UI, back=minimize via `CallHub`. One-to-one voice+video implemented; **not yet exercised device-to-device** (needs both installations registered to the dev service). E2EE is development-scoped ONLY (see docs/architecture/aether-calls.md). |
